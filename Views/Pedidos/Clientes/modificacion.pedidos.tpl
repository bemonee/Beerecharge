...
{block name="section_title"}
    <div class="float-left">
        <i class="fa fa-cart-plus fa-fw" aria-hidden="true"></i> Modificar Pedido
    </div>
...
{block name="section"}
    <div class="row">
        <div class="col-12">
            <form action="{$p_dir}/clients/orders/update/{$pedido->getIdPED()}/" method="POST">
                ...
                <div id="divLineas" class="col-12">
                    <div class='table-responsive'>
                        <table class="table table-sm table-bordered table-hover">
                          ...
                          <tfoot>
                            <tr>
                                <th colspan="5" class="text-right"><strong>Total:</strong> <span id="totalPedido">${$pedido->getValorTotal()}</span></th>
                            </tr>
                          </tfoot>
                          <tbody id="lineaPedido">
                              {foreach $pedido->getRecargas() as $r}
                                  {assign 'cerveza' $r->getTipoCerveza()}
                                  {assign 'producto' $r->getProducto()}
                                  {assign 'cant' $r->getCantidad()}
                                  <tr>
                                      <td>{$cerveza->getDescripcion()}</td>
                                      <td>{$producto->getDescripcion()}</td>
                                      <td>{$cant}</td>
                                      <td class='border-right-0' name="precioLinea">${$r->getValor()}</td>
                                      <td class='text-right border-left-0'>
                                          <a name="btnBorrarLinea">
                                              <i class='fa fa-minus-circle fa-lg fa-fw' aria-hidden='true'></i>
                                          </a>
                                      </td>
                                      <input type="hidden" name="linea_cerveza[]" value="{$cerveza->getIdTC()}">
                                      <input type="hidden" name="linea_producto[]" value="{$producto->getIdPROD()}">
                                      <input type="hidden" name="linea_cantidad[]" value="{$cant}">
                                  </tr>
                              {/foreach}
                          </tbody>
                        </table>
                ...
                <div id="divEntrega">            
                    ...
                    <div class="col-12">
                        <!-- Inline radio -->
                        {if is_null($pedido->getSucursal())}
                            {assign 'envio' 'checked'}
                            {assign 'sucursal' ''}
                            {assign 'divEnvio' ''}
                            {assign 'divRetiro' 'd-none'}
                            {assign 'objEnvio' $pedido->getEnvio()}
                            {assign 'direccion' $objEnvio->getDireccion()}
                            {assign 'fecha_entrega' $objEnvio->getFechaEntrega()}
                            {assign 'hora_entrega_min' $objEnvio->getRangoHoraMin()}
                            {assign 'hora_entrega_max' $objEnvio->getRangoHoraMax()}
                            {assign 'idSUC' ''}
                            {assign 'initMap' ''}
                        {else}
                            {assign 'envio' ''}
                            {assign 'sucursal' 'checked'}
                            {assign 'divEnvio' 'd-none'}
                            {assign 'divRetiro' ''}
                            {assign 'direccion' ''}
                            {assign 'fecha_entrega' ''}
                            {assign 'hora_entrega_min' ''}
                            {assign 'hora_entrega_max' ''}
                            {assign 'idSUC' $pedido->getSucursal()->getIdSUC()}
                            {assign 'initMap' '&callback=initMap'}
                        {/if}
                        <label class="radio-inline pmd-radio">
                            <input type="radio" name="entrega" id="inlineRadio1" value="envio" {$envio}>
                            <span for="inlineRadio1">Envio a domicilio</span> 
                        </label>
                        <label class="radio-inline pmd-radio ml-2">
                            <input type="radio" name="entrega" id="inlineRadio2" value="retiro" {$sucursal}>
                            <span for="inlineRadio2">Retitro en sucursal</span> 
                        </label>				
                    </div>
                    <div id="divEnvio" class="col-12 mt-3 {$divEnvio}">
                        <div class="row">
                            ...
                            <div class="col-4 mt-1">
                                <div class="md-form form-sm">
                                    ...
                                    <input type="text" class="form-control" id="fecha_entrega" name="fecha_entrega" value="{$fecha_entrega|date_format:"%d/%m/%Y"}">
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                entre las 
                                <span id="slider-padding-value-min">{$hora_entrega_min|default:'10:00'}</span> y
                                <span id="slider-padding-value-max">{$hora_entrega_max|default:'21:00'}</span>Hs
                    ...
                    <div id="divRetiro" class="col-12 mt-3 {$divRetiro}">
                        <div class='table-responsive'>
                            <table class="table table-sm table-bordered table-hover">
                              <thead>
                                <tr>
                              ...
                              <tbody>
                                {if isset($sucursales)}
                                    {foreach $sucursales as $s}
                                        {if ($s->getIdSUC() == $idSUC)}
                                            {assign 'sucursal_checked' 'checked'}
                                        {else}
                                            {assign 'sucursal_checked' ''}
                                        {/if}
                                    <tr>
                                      ...
                                      <td class="text-right border-left-0">
                                          <label class="radio-inline pmd-radio">
                                            <input type="radio" name="sucursal" value="{$s->getIdSUC()}" data-direccion='{$s->getDireccion()}' data-nombre="{$s->getDescripcion()}" {$sucursal_checked}>
                                          </label>
                                      </td>
                                    </tr>
                                    {/foreach}
...
<script async defer src="https://maps.googleapis.com/maps/api/js?key={$GOOGLE_MAPS_API_KEY}{$initMap}"></script>
...
$(document).ready(function() {
    ...
    noUiSlider.create(snapSlider, {
            start: [{$hora_entrega_min|truncate:2:""|default:'10'}, {$hora_entrega_max|truncate:2:""|default:'21'}],
            margin: 1,
            step: 1,
            connect: true,
            range: {
...
function initMap() {
    ...
    if($sucursales.length && map === undefined){
            ...
            geocoder.geocode( { 'address' : direccion, 'region' : 'AR' }, function( results, status ) {
                if( status == google.maps.GeocoderStatus.OK ) {
                    ...
                    if (infowindow){
                        ...
                        if('{$idSUC}' == $(obj).val()){ // Si es el que se selecciono previamente, disparo el evento para que localize
                            $(obj).trigger("change");
                        }
                    }
                }
